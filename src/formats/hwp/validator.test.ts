...
describe('validateHwp', () => {
  ...
  describe('Section F — Layer 6: Content Completeness', () => {
    ...
    it('throws TypeError when threshold is Infinity', async () => {
      await expect(
        validateHwpBuffer(await createTestHwpBinary({ paragraphs: ['hello'] }), {
          contentCoverageThreshold: Number.POSITIVE_INFINITY,
        }),
      ).rejects.toThrow(TypeError)
    })
  })
  ...
  describe('Section L — Layer 12: Border Fill References', () => {
    ...
    it('fails when cells exist but no BORDER_FILL records declared', async () => {
      const filePath = await writeTempHwp(
        ...
        'validator-l-border-fill-missing-docinfo',
      )
  ...
  describe('Section M — Layer 13: Picture References', () => {
  ...
  describe('Section N — Layer 14: Empty Paragraph Text', () => {
    it('fails when an empty paragraph contains only the paragraph-end marker', async () => {
      ...
      const filePath = await writeTempHwp(await buildHwpWithCustomSection0(section0), 'validator-n-empty-paragraph-text')
      const result = await validateHwp(filePath)
