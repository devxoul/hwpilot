...
export async function loadHwpx(fileBuffer: Uint8Array): Promise<HwpxArchive> {
  let zip: JSZip
  try {
    zip = await JSZip.loadAsync(Buffer.from(fileBuffer))
  } catch (err) {
    throw new Error(`Failed to parse HWPX file as ZIP — ${(err as Error).message}`)
  }
...
function validateHwpx(zip: JSZip): void {
